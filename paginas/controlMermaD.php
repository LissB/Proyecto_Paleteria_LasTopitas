...
<?php
include_once '../Cruds/CrudCMD.php';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<header class="top-navbar">
    ...
    <div class="container">
      ...
      <div class="text-center ba">
        <h1 class="titPrin">Registro de Merma Detalle</h1>
      </div>
...
<div class="container-fluid ">
  ...
  <div class="form-group">
      ...
      <select name="Producto_idProducto">
    ...
    <div class="form-group obtds">
      <label class="control-label tamla" for="cantidadMerma">Cantidad de productos:</label>
      <div class="col bg-info">
        <input type="text" class="form-control text-uppercase" name="cantidadMerma" placeholder="2"  maxlength="3"
        value="<?php if(isset($_GET['editar'])) echo $getROW['cantidadMerma'];  ?>" />
      </div>
    </div>
    <div class="form-group obtds">
      <label class="control-label tamla" for="motivoMerma">Motivo Merma:</label>
      <div class="col bg-info">
        <input type="text" class="form-control text-uppercase" name="motivoMerma" placeholder="El producto se perdió porque no se vendió en la fecha requerida" maxlength="100"
        value="<?php if(isset($_GET['editar'])) echo $getROW['motivoMerma'];  ?>" />
      </div>
    </div>
    <div class="form-group">	      
      <?php
...
<?php
  ...
  $filtro="SELECT iddetallecontrolmerma, Producto_idProducto, controlmerma_idcontrolMerma, motivoMerma, cantidadMerma, subTotalMerma, producto.nombre,producto.costoCompra FROM detallecontrolmerma, producto WHERE idcontrolMerma = '$id' AND detallecontrolmerma.Producto_idProducto= producto.idProducto ORDER BY detallecontrolmerma.cantidadMerma";
  $SQL = $MySQLiconn->query($filtro);*/
  $rs ="SELECT max(idcontrolMerma) as id from controlmerma"; $SQL=$MySQLiconn->query($rs);
 if ($row = $SQL->fetch_array()) { $id = trim($row[0]); }
 $filtro="SELECT producto.nombre, detallecontrolmerma.motivoMerma,detallecontrolmerma.cantidadMerma, producto.costoCompra, detallecontrolmerma.subTotalMerma,detallecontrolmerma.Producto_idProducto FROM producto, detallecontrolmerma, controlmerma WHERE controlmerma.idcontrolMerma= '$id' AND detallecontrolmerma.Producto_idProducto= producto.idProducto AND detallecontrolmerma.controlmerma_idcontrolMerma= controlmerma.idcontrolMerma";
 $SQL = $MySQLiconn->query($filtro); 
?>
 <h3 class="textoTa text-center">Listado de Productos perdidos</h3><BR></BR>
